...
namespace VindictusCraftingCostCalculator
{
	public class CustomWebClient : UserControl, INotifyPropertyChanged
	{
		...
		/// </summary>
		public ObservableCollection<CraftingMaterial> ReadSetFromDB(string setName, out bool set_found)
		{
			...
			try
			{
				document = htmlWeb.Load(baseURIDB + URISet + setName);
			}
			...
			if (document != null && !document.DocumentNode.InnerText.Contains("No results"))
			{
				...
				// The node that contains all items belonging to this set. They are displayed in a table, so we can directly go to this table.
				HtmlNode nodeWithSetItems = document.DocumentNode.SelectSingleNode("/html[1]/body[1]/div[1]/div[4]/div[1]/div[1]/div[3]/table[1]/tbody[1]");
				...
				foreach (HtmlNode nodeWithSingleItem in nodeWithSetItems.ChildNodes.Where(n => n.Name == "tr"))
				{
					...
					HtmlNode nodeWithItemInfo = nodeWithSingleItem.ChildNodes[5].ChildNodes[1].ChildNodes[0].ChildNodes[0];
					...
					HtmlNode nodeWithItemImage = nodeWithItemInfo.ChildNodes[0].ChildNodes[0].ChildNodes[0];
					string itemImageUrl = nodeWithItemImage.Attributes[0].Value;

					// Extract the gold info
					HtmlNode nodeWithGoldCost = nodeWithSingleItem.ChildNodes[3].ChildNodes[1];
